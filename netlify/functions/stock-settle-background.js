// ─── stock-settle-background — the Stocks tab's brain, clocked in SESSIONS ───
//
// UNSCHEDULED, AND THAT IS THE POINT. This used to carry the hourly cron
// itself, and Netlify does not route public HTTP to a scheduled function — so
// the only thing that could ever start a pass was the scheduler. Miss an
// evening and the tab shows nothing until the next fire, on the one surface
// whose entire job is having tomorrow's plan ready before the bell. A tool you
// cannot run is a tool that is broken every time it matters.
//
// So the schedule moved to stock-cron-background.js, a thirty-line shim that
// does nothing but POST here, and this became an ordinary background function:
// invocable by that shim, and invocable by the Run now button on the tab. The
// pass itself is unchanged and still decides its own type — a manual run at
// midnight settles the session that just finished, exactly as the 1am fire
// would have.
//
//
 ...
 * five instruments, paced at one request every two seconds against an
 * undocumented Yahoo endpoint. Repeat that and the sweep's own breaker trips on
 * a 429, a 403 or Yahoo's HTML block page, and a tripped breaker is not a
 * degraded tab, it is a dark one: no board, no flags, no plan for the open,
 * until the ban lifts. The header at the top of this file already argued that
 * concurrency is handled by the database — stock_flags_one_open_uidx makes a
...
async function ownerDenied(event) {
  ...
  try {
    ...
    return json(503, { error: "couldn't verify your session — try again in a moment" });
  }
  return null;
}
...
// Counts only on every path — the payload never leaves through this endpoint,
// and error DETAIL stays in the function log rather than going to an
// unauthenticated caller.
//
// CONCURRENCY. Two overlapping passes are possible now that a human can start
// one. The dangerous half is guarded by the database rather than by this code:
// stock_flags_one_open_uidx is a partial unique index on (ticker_id) where
// resolved_at is null, so a second pass cannot open a duplicate episode. The
// state row is last-write-wins over two passes computing the same thing from
// the same bars, which is a tie, not a race.
exports.handler = async (event) => {
  ...
  try {
    ...
    return json(200, { success: true, service: "stock-settle-background", counts, errorCount: errors.length });
  } catch (e) {
    console.error("stock-settle-background failed:", e);
    return json(200, { success: false, service: "stock-settle-background", error: String((e && e.message) || e) });
  }
};
