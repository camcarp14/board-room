...
import { IcPlus, IcCheck } from "../../ui/icons.jsx";
        ...
        return (
          <div key={`${name}${i}`}>
            <div style={{ display: "flex", alignItems: "baseline", justifyContent: "space-between", gap: 8 }}>
              ...
              {onPlaySection && (
                <button type="button" className="sec-link" style={{ background: "none", border: "none", cursor: "pointer" }}
        ...
        <div>
          ...
          {parsed.bad.length > 0 && (
            <div className="t-foot" style={{ color: "var(--amber)", marginTop: 6 }}>
              Can't read: {parsed.bad.slice(0, 6).join(", ")}{parsed.bad.length > 6 ? "…" : ""}. Those bars will be skipped.
            </div>
          )}
          {parsed.sections.length > 0 && (
            <div className="t-foot" style={{ color: "var(--green)", marginTop: 6 }}>
              {parsed.sections.length} line{parsed.sections.length === 1 ? "" : "s"}, {chartChords(parsed.sections).length} distinct chords.
            </div>
          )}
        </div>
...
export function SongsPanel({ isMobile, settings, updateSetting }) {
  ...
  const setStatus = async (song, status) => {
    if (status === "learning" && learningCount >= 3 && song.status !== "learning") {
  ...
  if (error) {
    return (
      <EmptyState icon="⚠️" title="Couldn't load your songs"
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
          ...
          <CellGroup>
            {list.map((s) => (
              <Cell key={s.id} title={s.title} sub={[s.artist, s.key, s.capo ? `capo ${s.capo}` : null, DIFFICULTY[s.difficulty] || null].filter(Boolean).join(" · ")}
                value={s.status === "polishing" || s.status === "owned" ? `${s.cleanRuns || 0}/3` : null}
      ...
      {editing && (
        <SongEditor isMobile={isMobile} song={editing === "new" ? null : editing}
          ...
          onSave={(s) => saveSong.mutateAsync({ ...s, id: editing === "new" ? crypto.randomUUID() : editing.id })}
          onDelete={del} />
      )}
